...
const TherapistScheduleTabs: FC<ITherapistScheduleTabsProps> = ({ schedules, therapist }) => {
  return (
    <div className="therapist-schedule">
      ...
      <div className="therapist-schedule__content">
        <Tabs
          tabNavsWrapper={(children) => <ul className="therapist-schedule__tabs">{children}</ul>}
        ...
        >
          {schedules.map((schedule) => (
            <TherapistScheduleTabItem items={schedule.items} key={schedule.day} />
          ))}
        </Tabs>
      </div>
    </div>
